/**
 * @param {object} allSongs - Objeto de armazenamento de todas as musicas
 * da aplicação com suas caracteristicas
 */

const allSongs = {
  aa000: {
    nameSong: "(Look Out) She's America",
    artist: "Otis McDonald",
    src: "/Astro/Backend/Assets-Songs/Audio/(Look Out)She'sAmerica.mp3",
    imgSong: "https://source.unsplash.com/featured/80x80",
    genero: "Rap",
    ritmo: "agitado",
  },

  /**
   * @param {atributes}  Ritmo_Genero -
   * Categorias usadas para a geração das listas genêricas,
   * podendo ser agrupadas com musicas com as mesmas
   * caracteristicas em um array
   */

  aa001: {
    nameSong: "6 Shots",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/6shots.mp3",
    imgSong: "https://source.unsplash.com/featured/80x79",
    genero: "Hip-Hop",
    ritmo: "intenso",
  },

  aa002: {
    nameSong: "A Slow Dream",
    artist: "Emily Sprague",
    src: "/Astro/Backend/Assets-Songs/Audio/ASlowDream.mp3",
    imgSong: "https://source.unsplash.com/featured/79x80",
    genero: "Instrumental",
    ritmo: "Calmo",
  },

  aa003: {
    nameSong: "Anxiety",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/Anxiety.mp3",
    imgSong: "https://source.unsplash.com/featured/81x80",
    genero: "Hip-Hop",
    ritmo: "moderado",
  },

  aa004: {
    nameSong: "Auld Lang Syne",
    artist: "E's Jammy Jams",
    src: "/Astro/Backend/Assets-Songs/Audio/AuldLangSyne.mp3",
    imgSong: "https://source.unsplash.com/featured/80x81",
    genero: "Jazz",
    ritmo: "dramatico",
  },

  aa005: {
    nameSong: "Body of Water",
    artist: "TrackTribe",
    src: "/Astro/Backend/Assets-Songs/Audio/BodyofWater.mp3",
    imgSong: "https://source.unsplash.com/featured/81x79",
    genero: "Instrumental",
    ritmo: "dramatico",
  },

  aa006: {
    nameSong: "Bounce",
    artist: "Audionautix",
    src: "/Astro/Backend/Assets-Songs/Audio/Bounce.mp3",
    imgSong: "/Assets/Images/",
    genero: "Dance",
    ritmo: "agitado",
  },

  aa007: {
    nameSong: "Brontosaurus",
    artist: "Topher Mohr and Alex Elena",
    src: "/Astro/Backend/Assets-Songs/Audio/Brontosaurus.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rock",
    ritmo: "moderado",
  },

  aa008: {
    nameSong: "Catch Me If I Fall",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/CatchMeIfIFall.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "agitado",
  },

  aa009: {
    nameSong: "Changing",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/Changing.mp3",
    imgSong: "/Assets/Images/",
    genero: "Hip-Hop",
    ritmo: "moderado",
  },

  aa0010: {
    nameSong: "Chasing",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/Chasing.mp3",
    imgSong: "/Assets/Images/",
    genero: "Hip-Hop",
    ritmo: "agitado",
  },

  aa0011: {
    nameSong: "Do or Die",
    artist: "Dougie Wood",
    src: "/Astro/Backend/Assets-Songs/Audio/DoorDie.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "moderado",
  },

  aa0012: {
    nameSong: "Dream It",
    artist: "TrackTribe",
    src: "/Astro/Backend/Assets-Songs/Audio/DreamIt.mp3",
    imgSong: "/Assets/Images/",
    genero: "Instrumental",
    ritmo: "dramatico",
  },

  aa0013: {
    nameSong: "Emotional Mess",
    artist: "Amy Lynn & The Honey Men",
    src: "/Astro/Backend/Assets-Songs/Audio/EmotionalMess.mp3",
    imgSong: "/Assets/Images/",
    genero: "Jazz",
    ritmo: "romantico",
  },

  aa0014: {
    nameSong: "Fingers",
    artist: "Otis McDonald",
    src: "/Astro/Backend/Assets-Songs/Audio/Fingers.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rap",
    ritmo: "moderado",
  },

  aa0015: {
    nameSong: "Greener Grass",
    artist: "Dyalla",
    src: "/Astro/Backend/Assets-Songs/Audio/GreenerGrass.mp3",
    imgSong: "/Assets/Images/",
    genero: "Eletrônica",
    ritmo: "intenso",
  },

  aa0016: {
    nameSong: "Gully Dreams",
    artist: "Hanu Dixit",
    src: "/Astro/Backend/Assets-Songs/Audio/GullyDreams.mp3",
    imgSong: "/Assets/Images/",
    genero: "Árabe",
    ritmo: "agitado",
  },

  aa0017: {
    nameSong: "Here it Comes",
    artist: "TrackTribe",
    src: "/Astro/Backend/Assets-Songs/Audio/HereitComes.mp3",
    imgSong: "/Assets/Images/",
    genero: "Instrumental",
    ritmo: "raiva",
  },

  aa0018: {
    nameSong: "HTML",
    artist: "Riot",
    src: "/Astro/Backend/Assets-Songs/Audio/HTML.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "moderado",
  },

  aa0019: {
    nameSong: "I Had a Feeling",
    artist: "TrackTribe",
    src: "/Astro/Backend/Assets-Songs/Audio/IHadaFeeling.mp3",
    imgSong: "/Assets/Images/",
    genero: "Eletrônica",
    ritmo: "intenso",
  },

  aa0020: {
    nameSong: "Ice & Fire",
    artist: "King Canyon",
    src: "/Astro/Backend/Assets-Songs/Audio/IceFire.mp3",
    imgSong: "/Assets/Images/",
    genero: "Hip-Hop",
    ritmo: "moderado",
  },

  aa0021: {
    nameSong: "It Was a Time",
    artist: "TrackTribe",
    src: "/Astro/Backend/Assets-Songs/Audio/ItWasaTime.mp3",
    imgSong: "/Assets/Images/",
    genero: "Instumental",
    ritmo: "sombrio",
  },

  aa0022: {
    nameSong: "I Wish I Knew",
    artist: "Otis McDonald",
    src: "/Astro/Backend/Assets-Songs/Audio/IWishIKnew.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "moderado",
  },

  aa0023: {
    nameSong: "Just Do It",
    artist: "RKVC",
    src: "/Astro/Backend/Assets-Songs/Audio/JustDoIt.mp3",
    imgSong: "/Assets/Images/",
    genero: "Instrumental",
    ritmo: "intenso",
  },

  aa0024: {
    nameSong: "Lawrence",
    artist: "TrackTribe",
    src: "/Astro/Backend/Assets-Songs/Audio/Lawrence.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "agitado",
  },

  aa0025: {
    nameSong: "Losing My Mind",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/LosingMyMind.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rap",
    ritmo: "dramatico",
  },

  aa0026: {
    nameSong: "My Way",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/MyWay.mp3",
    imgSong: "/Assets/Images/",
    genero: "Hip-Hop",
    ritmo: "raiva",
  },

  aa0027: {
    nameSong: "No Filter",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/NoFilter.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rock",
    ritmo: "intenso",
  },

  aa0028: {
    nameSong: "No Mercy",
    artist: "TrackTribe",
    src: "/Astro/Backend/Assets-Songs/Audio/NoMercy.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rock",
    ritmo: "intenso",
  },

  aa0029: {
    nameSong: "Retribution",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/Retribution.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rock",
    ritmo: "sombrio",
  },

  aa0030: {
    nameSong: "Rumba Sabor",
    artist: "Jimmy Fontanez",
    src: "/Astro/Backend/Assets-Songs/Audio/RumbaSabor.mp3",
    imgSong: "/Assets/Images/",
    genero: "Folk",
    ritmo: "agitado",
  },

  aa0031: {
    nameSong: "Steamboat",
    artist: "Mini Vandals",
    src: "/Astro/Backend/Assets-Songs/Audio/Steamboat.mp3",
    imgSong: "/Assets/Images/",
    genero: "Folk",
    ritmo: "dramatico",
  },

  aa0032: {
    nameSong: "The Movement of India",
    artist: "Aakash Gandhi",
    src: "/Astro/Backend/Assets-Songs/Audio/TheMovementofIndia.mp3",
    imgSong: "/Assets/Images/",
    genero: "Country",
    ritmo: "moderado",
  },

  aa0033: {
    nameSong: "Till I'm on Top",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/Tilli'monTop.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rap",
    ritmo: "raiva",
  },

  aa0034: {
    nameSong: "Underground Academy",
    artist: "Hanu Dixit",
    src: "/Astro/Backend/Assets-Songs/Audio/UndergroundAcademy.mp3",
    imgSong: "/Assets/Images/",
    genero: "Country",
    ritmo: "intenso",
  },

  aa0035: {
    nameSong: "Waterpark",
    artist: "Freedom Trail Studio",
    src: "/Astro/Backend/Assets-Songs/Audio/Waterpark.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "agitado",
  },

  aa0036: {
    nameSong: "What You Gonna Be",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/WhatYouGonnaBe.mp3",
    imgSong: "/Assets/Images/",
    genero: "Hip-Hop",
    ritmo: "raiva",
  },

  aa0037: {
    nameSong: "You Will Never See Me Coming",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/YouWillNeverSeeMeComing.mp3",
    imgSong: "/Assets/Images/",
    genero: "Eletrônica",
    ritmo: "raiva",
  },

  aa0038: {
    nameSong: "No Time",
    artist: "Lastlings",
    src: "http://physical-authority.surge.sh/music/1.mp3",
    imgSong:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQBuflyiNrBSP0o-7KM9YqUsAZN8HIx7VCExFerUh4FMg&s",
    genero: "Eletrônica",
    ritmo: "agitado",
  },

  aa0039: {
    nameSong: "blinding lights",
    artist: "the weekend",
    src: "http://physical-authority.surge.sh/music/2.mp3",
    imgSong:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRcz6vl7dthSyfdNh6Eg4ZrcAYSieDSn7yCmQ&usqp=CAU",
    genero: "Pop",
    ritmo: "moderado",
  },

  aa0040: {
    nameSong: "джованна",
    artist: "enrasta",
    src: "http://physical-authority.surge.sh/music/3.mp3",
    imgSong: "https://source.unsplash.com/featured/300x300",
    genero: "Árabe",
    ritmo: "Calma",
  },

  aa0041: {
    nameSong: "A Year Ago",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/AYearAgo.mp3",
    imgSong: "/Assets/Images/Astro_main-caps/7.png",
    genero: "Pop",
    ritmo: "moderado",
    album: "Lost In Space",
  },

  aa0042: {
    nameSong: "Addict",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/Addict.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rock",
    ritmo: "raiva",
    album: "Lost In Space",
  },

  aa0043: {
    nameSong: "AsYouFadeAway",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/As You Fade Away.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "agitado",
    album: "Lost In Space",
  },

  aa0044: {
    nameSong: "Coast",
    artist: "Anno Domini Beats",
    src: "/Astro/Backend/Assets-Songs/Audio/Coast.mp3",
    imgSong: "/Assets/Images/Astro_main-caps/15.png",
    genero: "Hip-Hop",
    ritmo: "moderado",
    album: "Abstraction",
  },

  aa0045: {
    nameSong: "Contrast",
    artist: "Anno Domini Beats",
    src: "/Astro/Backend/Assets-Songs/Audio/Contrast.mp3",
    imgSong: "/Assets/Images/Astro_main-caps/15.png",
    genero: "Instrumental",
    ritmo: "dramatico",
    album: "Abstraction",
  },

  aa0046: {
    nameSong: "Crazy",
    artist: "Patrick PatriKios",
    src: "/Astro/Backend/Assets-Songs/Audio/Crazy.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "agitado",
    album: "Future",
  },

  aa0047: {
    nameSong: "How's It Supposed to Feel",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/How'sItSupposedtoFeel.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "intenso",
    album: "Lost In Space",
  },

  aa0048: {
    nameSong: "Just Breathing",
    artist: "NEFFEX",
    src: "/Astro/Backend/Assets-Songs/Audio/JustBreathing.mp3",
    imgSong: "/Assets/Images/",
    genero: "Pop",
    ritmo: "agitado",
    album: "Lost In Space",
  },

  aa0049: {
    nameSong: "Pressure Cooker",
    artist: "Jeremy Korpas",
    src: "/Astro/Backend/Assets-Songs/Audio/PressureCooker.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rock",
    ritmo: "intenso",
    album: "",
  },

  aa0050: {
    nameSong: "Sharp",
    artist: "Jeremy Korpas",
    src: "/Astro/Backend/Assets-Songs/Audio/Sharp.mp3",
    imgSong: "/Assets/Images/",
    genero: "Rock",
    ritmo: "raiva",
    album: "",
  },
};

export default allSongs;
